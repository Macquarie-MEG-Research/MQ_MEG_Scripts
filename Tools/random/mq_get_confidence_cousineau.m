function [yMean, yCI95] = mq_get_confidence_cousineau(y)
%
...
yMean_adjusted = zeros(size(y));

for i = 1:size(y,1)
    yMean_adjusted(i,:) = (y(i,:) - mean(y(i,:),2)+yMean);
end

mean(y,2)

ySEM = std(yMean_adjusted)./sqrt(N);                 % Compute ?Standard Error Of The Mean? Of All Experiments At Each Value Of ?x?
